            ...
            Some((ipv4_packet_data, _ipaddr)) => {
                ...
                if receive_protocol == IpNextHeaderProtocols::Icmp {
                    ...
                    if ipv4_packet_data.get_source().to_string() != "127.0.0.1"{
                        let ipv4_checksum = check_checksum(fixed_ipv4_header.clone())?;
                    }
                    
                    if receive_icmp_type == 11 && receive_icmp_code == 0 {
                        // Time Exceededの場合
                        ...
                        let icmp_checksum = check_checksum(fixed_payload.clone())?;

                        // 内部の元のICMPパケットを解析
                        let original_ip_header_length = ((receive_payload[8] & 0x0F) * 4) as usize;
                        let original_icmp_packet = &receive_payload[8 + original_ip_header_length..];
                        let original_icmp_identification = u16::from_be_bytes([original_icmp_packet[4],original_icmp_packet[5]]);
                        let original_icmp_seq = u16::from_be_bytes([original_icmp_packet[6],original_icmp_packet[7]]);
                        if original_icmp_identification != identification || original_icmp_seq != icmp_seq {
                            continue;
                        }

                    } else if receive_icmp_type == 0 && receive_icmp_code == 0 {
                        // Echo Replyの場合

                        let icmp_checksum = check_checksum(receive_payload.to_vec())?;
                        let receive_icmp_identification = u16::from_be_bytes([receive_payload[4],receive_payload[5]]);
                        if receive_icmp_identification != identification || receive_icmp_seq != icmp_seq {
                            continue;
                        }
