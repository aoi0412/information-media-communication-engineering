...
fn main()-> Result<(), Box<dyn std::error::Error>> {
    ...
    let received_type_num = Arc::new(AtomicU8::new(0));
    let is_finished = Arc::new(AtomicBool::new(false));
    println!("search mtu to {}, first mtu is {} ", destination_ipv4, first_mtu);
    println!("");

    // ---パケット受信スレッドを作成---
    let pcap_ready_clone = Arc::clone(&pcap_ready);
    let sent_packets_clone = Arc::clone(&sent_packets);
    let received_type_num_clone = Arc::clone(&received_type_num);
        ...
        loop {
            ...
            for packet_info in sent_packets.iter() {
                ...
                if packet_formatted_data.icmp_info.icmp_type == 3 && packet_formatted_data.icmp_info.icmp_code == 4{
                    ...
                    let sended_ttl = sended_ipv4_packet.ttl;
                    ...
                    if let Some((old_max, new_max)) = mtu_updated {
                        println!("--- ×{} ⚪︎{} ---", old_max, new_max);
                    }



                    
                    // received_type_numを取得したICMPタイプの番号に変更
                    received_type_num_clone.store(3, Ordering::SeqCst);
                ...
                } else if packet_formatted_data.icmp_info.icmp_type == 11 && packet_formatted_data.icmp_info.icmp_code == 0{
                    // ICMPペイロードから元の送信パケットの識別子とシーケンス番号を取得
                    let analyzed_icmp_payload = analyze_err_icmp_payload(&packet_formatted_data.icmp_bytes).unwrap();
                    let received_icmp_packet = analyzed_icmp_payload.received_icmp_packet;
                    let sended_ipv4_packet = analyzed_icmp_payload.sended_ipv4_packet;
                    ...
                    // received_type_num_clone.store(true, Ordering::SeqCst);
                    let current_max = current_max_mtu_clone.lock().unwrap();
                    println!("{} {} {:.3} ms ", sended_icmp_seq, dest_ip, rtt.as_secs_f64() * 1000.0);
                    

                    
                    // received_type_numを取得したICMPタイプの番号に変更
                    received_type_num_clone.store(11, Ordering::SeqCst);
                    analyzed_sent_packet.push(SentPacketInfo{
                        identification: packet_info.identification,
                        icmp_seq: packet_info.icmp_seq,
                        current_max_mtu: *current_max,
                    ...
                    println!("{} {} {:.3} ms", received_icmp_seq, dest_ip, rtt.as_secs_f64() * 1000.0);
                
                    // 正常終了を通知
                    received_type_num_clone.store(0, Ordering::SeqCst);
    ...
    loop {
        ...
        match send_packet(icmp_count, destination_ipv4, current_ttl, payload) {
            Ok((identification, icmp_seq, start_instant)) => {
                ...
                // received_type_numがtrueになるまで待機し、trueだったらfalseにして次のパケットを送信する
                while received_type_num.load(Ordering::SeqCst) == 0 {
                    if is_finished.load(Ordering::SeqCst) {
                        break;
                    }
                ...
                if received_type_num.load(Ordering::SeqCst) != 3 {
                    icmp_count += 1;
                    current_ttl += 1;
                }
                received_type_num.store(0, Ordering::SeqCst);
            }
            Err(err) => {
